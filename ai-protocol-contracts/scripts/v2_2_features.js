// v2.2 features setup script: setups features for
//   - SophiaBeing

// Run: npx hardhat run --network rinkeby ./scripts/v2_2_features.js
...
async function main() {
	...
	// setup features for SophiaBeing
	if(conf.SophiaBeing) {
		...
		console.log("Connected to SophiaBeing (AletheaNFT) at %o:", conf.SophiaBeing);
		console.table([
			{"key": "Name", "value": name},
		...
		if(features.isZero()) {
			console.log('Enabling "FEATURE_ALL" %o for SophiaBeing (AletheaNFT) %o', FEATURE_ALL, conf.SophiaBeing);
			await instances.SophiaBeing.updateFeatures(FEATURE_ALL);
		}
	}
	else {
		console.log("SophiaBeing (AletheaNFT) is not deployed, skipping");
	}
