...
/**
 * prints AccessControl info (features, deployer role)
 * @deprecated use print_contract_details instead
 */
async function print_acl_details(a0, abi, address, a1) {
	const web3_contract = new web3.eth.Contract(abi, address);
	const features = toBN(await web3_contract.methods.features().call());
	const r0 = toBN(await web3_contract.methods.getRole(a0).call());
	const r1 = toBN(a1? await web3_contract.methods.getRole(a1).call(): 0);
	console.log("successfully connected to AccessControl-based contract at %o", address);
	console.table([
		{"key": "Features", "value": features.toString(2)}, // 2
		{"key": "Deployer Role", "value": r0.toString(16)}, // 16
		{"key": "A1 Role", "value": r1.toString(16)}, // 16
