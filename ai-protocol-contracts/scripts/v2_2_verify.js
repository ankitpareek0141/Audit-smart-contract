// v2.2 source code verification script: verifies on Etherscan
//   - SophiaBeing

// Run: npx hardhat run --network rinkeby ./scripts/v2_2_verify.js
...
async function main() {
	...
	// verify SophiaBeing (AletheaNFT) source code
	if(conf.SophiaBeing) {
		console.log("Submitting SophiaBeing (AletheaNFT) source code for verification at %o", conf.SophiaBeing);
		verification_tasks.push(hre.run("verify:verify", {
			address: conf.SophiaBeing,
			constructorArguments: ["Sophia beingAI iNFT", "SOP"],
		}));
	}
