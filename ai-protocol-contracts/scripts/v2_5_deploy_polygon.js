// v2.5 deployment script: deploys into Polygon (Mumbai)
//   - PolygonAliERC20v2
//   - PolygonCharacter (WhitelabelNFT)
//   - CharacterFactory (NFTFactory)

// Run: npx hardhat run --network mumbai ./scripts/v2_5_deploy_polygon.js
...
async function main() {
	...
	if(conf.PolygonAliERC20v2) {
		...
		console.log("deploying PolygonAliERC20v2, H0 = %o", A0);
		const token = await PolygonAliERC20v2.deploy(A0);
		instances.PolygonAliERC20v2 = await token.deployed();
		conf.PolygonAliERC20v2 = instances.PolygonAliERC20v2.address;
		console.log("PolygonAliERC20v2 deployed to %o", conf.PolygonAliERC20v2);

		// burn the tokens (minted on the deployment)
/*
		if(S0.eq(await instances.PolygonAliERC20v2.balanceOf(A0))) {
			console.log("burning %o tokens from %o", S0.toString(), A0);
			await instances.PolygonAliERC20v2.burn(A0, S0);
		}
*/
	}

	// link/deploy PolygonCharacter (WhitelabelNFT)
	...
	if(conf.PolygonCharacter) {
		...
		console.log("deploying PolygonCharacter (WhitelabelNFT)");
		const token = await WhitelabelNFT.deploy("mycharacter.ai", "MYAI");
		instances.PolygonCharacter = await token.deployed();
		conf.PolygonCharacter = instances.PolygonCharacter.address;
		console.log("PolygonCharacter (WhitelabelNFT) deployed to %o", conf.PolygonCharacter);
	}

	// link/deploy CharacterFactory (NFTFactory)
	const NFTFactory = await hre.ethers.getContractFactory("NFTFactory");
	if(conf.CharacterFactory) {
		console.log("Connecting to CharacterFactory (NFTFactory) at %o", conf.CharacterFactory);
		instances.CharacterFactory = await NFTFactory.attach(conf.CharacterFactory);
		...
		console.log("deploying CharacterFactory (NFTFactory)");
		const factory = await NFTFactory.deploy();
		instances.CharacterFactory = await factory.deployed();
		conf.CharacterFactory = instances.CharacterFactory.address;
		console.log("CharacterFactory (NFTFactory) deployed to %o", conf.CharacterFactory);
	}
