...
// run ERC20Shares tests
contract("ERC20Shares (standalone)", function(accounts) {
	...
	function main_test_suite(has_holders_fee = true) {
		...
		it("paymentToken gets set correctly", async function() {
			expect(await shares.getPaymentToken()).to.be.equal(payment_token.address);
		});
		...
		function buying_the_shares(change = 0) {
			...
			describe("buying the very first share", function() {
				...
				describe("succeeds otherwise", function() {
					...
					describe("buying next shares", function() {
						...
						it("fails if ERC20 payment token supplied is zero", async function() {
							await expectRevert(shares.buyShares(amount, {from: buyer}), "transfer amount exceeds allowance");
						});
						it("fails if tx contains ETH", async function() {
							await payment_token.approve(shares.address, S0, {from: buyer});
							await expectRevert(shares.buyShares(amount, {
								from: buyer,
								value: 1
							}), "only payment in ERC20 token is expected");
						});
						describe("succeeds otherwise", function() {
							...
							let receipt;
							beforeEach(async function() {
								buyer_tracker = await balance.tracker(buyer, payment_token);
								shares_tracker = await balance.tracker(shares.address, payment_token);
								protocol_fee_tracker = await balance.tracker(protocol_fee_destination, payment_token);
								holders_fee_tracker = await balance.tracker(holders_fee_destination, payment_token);
								issuer_tracker = await balance.tracker(issuer, payment_token);
								buy_price = await shares.getBuyPrice(amount);
								protocol_fee = buy_price.mul(protocol_fee_percent).div(ETH);
								...
								await payment_token.approve(shares.address, value.add(change), {from: buyer});
								receipt = await shares.buyShares(amount, {from: buyer});
							});
							it('"Trade" event is emitted', async function() {
								expectEvent(receipt, "Trade", {
									beneficiary: buyer,
							...
							describe("issuer can sell the first share back", function() {
								...
								beforeEach(async function() {
									...
									protocol_fee = sell_price.mul(protocol_fee_percent).div(ETH);
			...
			describe("buying several first shares", function() {
				beforeEach(async function() {
					// give some tokens to the issuer
					await payment_token.transfer(issuer, S0.divn(2), {from: H0});
				});
				...
				it("fails if ERC20 payment token supplied is zero", async function() {
					await expectRevert(shares.buyShares(init_amount, {from: issuer}), "transfer amount exceeds allowance");
				});
				describe("succeeds otherwise", function() {
					let shares_tracker, protocol_fee_tracker, holders_fee_tracker, issuer_tracker;
					...
					beforeEach(async function() {
						...
						protocol_fee = first_price.mul(protocol_fee_percent).div(ETH);
						...
						await payment_token.approve(shares.address, value.add(change), {from: issuer});
					...
					describe("buying next shares", function() {
						...
						it("fails if ERC20 payment token supplied is zero", async function() {
							await expectRevert(shares.buyShares(buy_amount, {from: buyer}), "transfer amount exceeds allowance");
						});
						describe("succeeds otherwise", function() {
							let buyer_tracker, shares_tracker, protocol_fee_tracker, holders_fee_tracker, issuer_tracker;
							...
							beforeEach(async function() {
								...
								protocol_fee = next_price.mul(protocol_fee_percent).div(ETH);
							...
							describe("selling some shares back", function() {
								...
								let receipt;
								beforeEach(async function() {
									seller_tracker = await balance.tracker(seller, payment_token);
									shares_tracker = await balance.tracker(shares.address, payment_token);
									protocol_fee_tracker = await balance.tracker(protocol_fee_destination, payment_token);
									holders_fee_tracker = await balance.tracker(holders_fee_destination, payment_token);
									issuer_tracker = await balance.tracker(issuer, payment_token);
		...
		describe("buying the shares without change (ERC20 payment token supplied is exact)", function() {
			buying_the_shares(0);
		});
		describe("buying the shares with change (ERC20 payment token supplied exceeds what is required)", function() {
	...
	function disable_holders_fee_test_suite() {
		describe("when several first shares are bought", function() {
			...
			beforeEach(async function() {
				...
				await payment_token.approve(shares.address, value, {from: issuer});
				receipt = await shares.buyShares(init_amount, {from: issuer});
			});
			it('"Trade" event is emitted', async function() {
				expectEvent(receipt, "Trade", {
			...
			describe("when the shares holders fee is disabled", function() {
				...
				describe("buying more shares after the shares holders fee was disabled", function() {
					...
					beforeEach(async function() {
						...
						value = await shares.getBuyPriceAfterFee(buy_amount);
						effective_value = price.add(protocol_fee).add(holders_fee).add(subject_fee);
						await payment_token.approve(shares.address, value, {from: buyer});
						receipt = await shares.buyShares(buy_amount, {from: buyer});
					});
					it('"Trade" event is emitted', async function() {
						expectEvent(receipt, "Trade", {
	...
	describe("when standalone shares contract is deployed with malicious holders fee destination", function() {
		beforeEach(async function() {
			holders_fee_distributor = await deploy_holders_rewards_distributor(a0, payment_token, ZERO_ADDRESS, true);
			({shares} = await deploy_shares_ERC20(
				a0,
				payment_token,
				issuer,
				undefined,
		...
		it("buying the share fails", async function() {
			await expectRevert(shares.buyShares(1, {from: issuer, gas: 1_000_000}), "out of gas");
		});
	});
});
