...
contract("buy/sell simulation", function(accounts) {
	...
	before(async function() {
		const price = await get_buy_price_after_fee(0, s0, protocol_fee_percent, holders_fee_percent, subject_fee_percent);
		...
		for(let i = 0; i < influencers.length; i++) {
			...
			const {
				shares,
				distributor,
			} = await factory_deploy_shares(influencers[i].address, factory, subject, undefined, undefined, s0, price);
	...
	it(`simulation main loop (steps = ${SIM_STEPS}) [ @skip-on-coverage ]`, async function() {
		...
		async function dummy_step(trader) {
			...
			if(Math.random() < 0.5) {
				...
				// agent wants to buy
					...
					await shares.buyShares(amount, {value: price, from: trader.address});
		...
		async function print_portfolios(traders) {
			...
			for(let i = 0; i < traders.length; i++) {
				...
				portfolios[i] += print_amt(await web3.eth.getBalance(trader.address));
			}
...
*/
		...
		console.log("protocol fees collected: %o", print_amt(await web3.eth.getBalance(protocol_fee_destination)));

		const holders_fees_collected = new BN(0);
		for(const distributor of deployed_distributors) {
			holders_fees_collected.iadd(new BN(await web3.eth.getBalance(distributor.address)));
		}
		...
		for(let i = 0; i < participants.length; i++) {
			const participant = participants[i];
			console.log("%d):\t%o", i + 1, print_amt(await web3.eth.getBalance(participant.address)));
		}
		console.log("final bonding curve balances:");
		for(let i = 0; i < deployed_curves.length; i++) {
			const shares = deployed_curves[i];
			console.log("%d):\t%o", i + 1, print_amt(await web3.eth.getBalance(shares.address)));
		}
		...
		for(let i = 0; i < deployed_distributors.length; i++) {
			...
			console.log("%d):\t%o", i + 1, print_amt(await web3.eth.getBalance(distributor.address)));
		}
		...
		for(let i = 0; i < deployed_curves.length; i++) {
			const shares = deployed_curves[i];
			expect(await web3.eth.getBalance(shares.address), `shares ${i + 1} balance`).to.be.bignumber.below(one_gwei);
		}
		for(let i = 0; i < deployed_distributors.length; i++) {
			const distributor = deployed_distributors[i];
			expect(await web3.eth.getBalance(distributor.address), `distributor ${i + 1} balance`).to.be.bignumber.below(one_gwei);
		}
	});
});
