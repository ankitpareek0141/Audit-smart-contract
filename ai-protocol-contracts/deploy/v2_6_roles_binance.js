// run: npx hardhat deploy --network binance_testnet --tags v2_6_roles_binance
...
module.exports = async function({deployments, getChainId, getNamedAccounts, getUnnamedAccounts}) {
	...
	// ERC721 <- NFT Factory role injection
	{
		...
		const {address: nft_factory_address} = await deployments.get("BinanceArtFactory");
		...
		if(!r1.eq(requested_role)) {
			...
			console.log(
				"BinanceTestArt.updateRole(%o, 0x%o): %o",
				nft_factory_address,
	...
	// ERC721 <- NFT Factory role injection
	{
		...
		if(!r1.eq(requested_role)) {
			...
			console.log(
				"BinanceVitalikArt.updateRole(%o, 0x%o): %o",
				nft_factory_address,
				requested_role.toString(16),
				receipt.transactionHash
			);
	...
	// NFT Factory <- service wallets role injection
	{
		...
		for(const service_wallet of serviceWallets) {
			...
			if(!r1.eq(requested_role)) {
				...
				console.log(
					"BinanceArtFactory.updateRole(%o, 0x%o): %o",
					service_wallet,
					requested_role.toString(16),
					receipt.transactionHash
				);
...
module.exports.tags = ["v2_6_roles_binance", "roles_binance", "v2_6"];
