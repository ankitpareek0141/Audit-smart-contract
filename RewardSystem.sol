...
contract RewardSystem is UpgradeableAccessControl {
	 ...
	 */
	function updateClaimedReward(address userAddress, uint256 value) private {
		// update the data at the "active" last storage slot
		updateClaimedReward(claimedRewards.length - 1, userAddress, value);
	}
	 ...
	 * @param value the reward value to set
	 */
	function updateClaimedReward(uint256 index, address userAddress, uint256 value) private {
	...
	function claimReward(address payable _to, uint256 _totalReward, bytes32[] memory _proof) external {
		...
		updateClaimedReward(_to, _totalReward);
